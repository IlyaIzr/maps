...
export const MapArea = () => {
  ...
  const { geodata: appGeodata, isMapLoaded } = useSelector(
    (state) => state.map
  );
  const d = useDispatch();
  ...
  useEffect(() => {
    ...
    mapOnLoad(
      ...
      app.theme,
      d,
      registerCitiesBanner.bind(this, d, history, themesCbStore)
    );
  ...
  useEffect(() => {
    ...
    const moveCb = mapOnMove(map.current, d, {
      mode: app.mode,
      tag: app.tagModeTag,
    });
  ...
  useEffect(() => {
    map.current &&
      isMapLoaded &&
      setMapData(map.current, appGeodata, RATED_LAYER_SRC);
    ...
    if (app.mode === "watch") {
      ...
      if (res.status !== "OK")
        return setToast(d, {
          ...
          message: res.msg + "#mp1" || JSON.stringify(res),
        });
      geoJson = geoJsonFromResponse(res.data);
    } else if (app.mode === "tags") {
      const res = await getTagPlaces(
        app.tagModeTag,
        x - initRange,
        x + initRange,
        y - initRange,
      ...
      if (res.status !== "OK")
        return setToast(d, {
          title: TEXT.networkError,
          message: res.msg + "#mp2" || JSON.stringify(res),
        });
      geoJson = geoJsonFromResponse(res.data);
    } else {
...
export function flyToFeature(map, feature, zoom = 16, speed = 0.5) {
  ...
  map.flyTo({ center: [lng - 0.001, lat + 0.0005], zoom, speed });
  return [lng, lat];
}
